...
export class GlobalService {
  ...
  private _adsModalData: unknown = null;
  private _itemsMap = new BehaviorSubject<ItemsMap | null>(null);
  private _globalParams = new BehaviorSubject<GlobalParams | null>(null);
  private _navbar = new BehaviorSubject<NavbarItem[] | null>(null);
  private _brands = new BehaviorSubject<Brand[] | null>(null);
    ...
    return this.http
      ...
      .pipe(
        tap((data) => {
          ...
          this._navbar.next(data.navbar?.navbar);
  ...
  setTitle(title: string) {
    try {
      let explo = title.trim().split(' ');

      explo = explo.map((word: string) => {
        return word
          ? word[0].toUpperCase() + word.substring(1).toLowerCase()
          : '';
      });

      const titleString = explo.join(' ') + ' | Trokaí';

      this.title.setTitle(titleString);
      this.meta.removeTag('name="og:title"');
      this.meta.addTag({ name: 'og:title', content: titleString });
      this.meta.removeTag('name="twitter:title"');
      this.meta.addTag({ name: 'twitter:title', content: titleString });
    } catch {
      /* intentional */
    }
  }
  ...
  setMetaDescription(content: string) {
    ...
    //  Open Graph Protocol description tag
    this.meta.removeTag('property="og:description"');
    this.meta.addTag({ property: 'og:description', content: content });
  ...
  verifyEmail(email: string, token: string) {
    return lastValueFrom(
      this.http.post(environment.urlApi + '/users/verify-email', {
        email,
        token,
      }),
    );
  ...
  getAdsModalData() {
    return this._adsModalData;
  }

  setAdsModalData(data: unknown) {
    this._adsModalData = data;
  }

  sendContactMail(name: string, email: string, message: string) {
    return lastValueFrom(
      this.http.post(environment.urlApi + '/contact-form/site', {
        name,
        email,
        message,
      }),
  ...
  base64ToBlob(base64Data: string, contentType: string) {
    ...
    const byteCharacters = this.browserRef.window?.atob(base64Data);
    if (!byteCharacters) return new Blob([], { type: contentType });
  ...
  public getListaBancos(): BasicModel[] {
    return [
